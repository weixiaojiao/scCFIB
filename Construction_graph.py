...
def load_data(data_path, is_NE=True, n_clusters=20, K=None):
	# Get data
	DATA_PATH = data_path
	...
	if is_NE:
		...
		method = 'pearson'
	adj = getGraph(features, L, K, method)
...
# Read data
A, X, cells, genes = load_data(data_path, args.is_NE, n_clusters)
pd.DataFrame(A).to_csv('data/' + dataset_str + '/'+dataset_str+'_graph.txt', index=False, header=None)
X = pd.DataFrame(X.T, columns=cells, index=genes)
X.to_csv('data/' + dataset_str + '/'+dataset_str+'_Normalized.tsv', index=True, header=True,sep="\t")
print('Done')